...
/* Finished implementing the setup_stack() function, as specified
   in the PintDoc. */
static bool
      ...
      if (success)
      {
        *esp = PHYS_BASE;
        ...
        for (token = file_name; token != NULL; token = strtok_r (NULL, " ", save_ptr))
        {
          ...
          *esp -= sizeof(char)*x;
          memcpy((char *)*esp,token,x);
        }

        char * tmp = *esp;
        *esp -= (size_t)(*esp)%4;
        char *tmp2 = 0;
        memcpy(*esp,&tmp2,(size_t)tmp-(size_t)*esp);

		*esp -= sizeof(char *);
		memcpy(*esp,&tmp2,sizeof(char *));

        *esp -= sizeof(char *);
        int num=0;
        while((void *)tmp < PHYS_BASE)
        {
            num++;
            *esp-=sizeof(char *);
            memcpy((char *)*esp,&tmp,sizeof(char *));
            tmp+=sizeof(char)*(strlen(tmp)+1);

        }
        char ** tmp3 = *esp;
        *esp-=sizeof(char **);
        memcpy(*esp,&tmp3,sizeof(char **));
